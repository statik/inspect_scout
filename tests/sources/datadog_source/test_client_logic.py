...
class TestPagination:
    ...
    @pytest.mark.asyncio
    async def test_naive_datetimes_treated_as_utc(self) -> None:
        """Naive datetimes get UTC timezone before serialization."""
        empty_response = {
            "data": [],
            "meta": {"page": {}},
        }
        ...
        naive_from = datetime(2026, 2, 1)
        naive_to = datetime(2026, 2, 28)

        await client.list_spans(from_time=naive_from, to_time=naive_to)

        params = mock_http.get.call_args_list[0].kwargs["params"]
        assert params["filter[from]"] == "2026-02-01T00:00:00+00:00"
        assert params["filter[to]"] == "2026-02-28T00:00:00+00:00"


class TestIterSpanPages:
    """Direct tests for the iter_span_pages async generator."""

    @pytest.mark.asyncio
    async def test_single_page_yields_one_list(self) -> None:
        """Single-page response yields exactly one list of spans."""
        response_data = {
            "data": [
                {"attributes": {"span_id": "s1"}},
                {"attributes": {"span_id": "s2"}},
        ...
        assert pages[0][0]["span_id"] == "s1"

    @pytest.mark.asyncio
    async def test_multi_page_yields_separate_lists(self) -> None:
        """Multi-page response yields one list per page with correct cursors."""
        page1_data = {
            "data": [{"attributes": {"span_id": "s1"}}],
        ...
        mock_resp_1.json.return_value = page1_data
        ...
        mock_http = AsyncMock()
        mock_http.get = AsyncMock(side_effect=[mock_resp_1, mock_resp_2])
        ...
        assert len(pages) == 2
        assert pages[0][0]["span_id"] == "s1"
        assert pages[1][0]["span_id"] == "s2"

        second_call_params = mock_http.get.call_args_list[1][1]["params"]
        assert second_call_params["page[cursor]"] == "cursor-1"

    @pytest.mark.asyncio
    async def test_empty_page_not_yielded(self) -> None:
        """Pages with no data items are skipped."""
    ...
    @pytest.mark.asyncio
    async def test_stops_without_next_cursor(self) -> None:
        """Pagination stops when no next cursor is provided."""
        response_data = {
            "data": [{"attributes": {"span_id": "s1"}}],
            "meta": {"page": {}},
        }
