...
# Number of consecutive absent pages before a buffered trace is flushed.
# A grace period of 1 handles the case where a trace's spans appear on
# non-adjacent pages in the API's time-ordered results (e.g. pages 1 and 3).
# Traces absent for more than this many pages are flushed as-is. Increase
# if production data shows traces spanning wider page gaps.
_STALE_PAGE_THRESHOLD = 1
...
async def _from_query(
    ...
    when it has been absent from more than ``_STALE_PAGE_THRESHOLD``
    consecutive pages (default 1). This grace period handles traces
    whose spans appear on non-adjacent pages in time-ordered results.
    ...
    pages_unseen: dict[str, int] = defaultdict(int)
    count = 0
    ...
    async for page in client.iter_span_pages(
        ...
        # Track consecutive pages each buffered trace has been absent from.
        for tid in page_trace_ids:
            pages_unseen.pop(tid, None)
        for tid in traces:
            if tid not in page_trace_ids:
                pages_unseen[tid] += 1

        # Flush traces absent for more than the grace period.
        completed = [
            tid for tid in traces if pages_unseen.get(tid, 0) > _STALE_PAGE_THRESHOLD
        ]
        for tid in completed:
            ...
            if transcript and _matches_trace_filter(
                ...
                if result is not None:
                    yield result
                    count += 1
                    # Closing the generator discards any in-flight HTTP
                    # request from iter_span_pages; this wastes at most
                    # one API call but avoids over-fetching.
                    if limit and count >= limit:
                        return

    # Yield remaining buffered traces (from the last pages)
